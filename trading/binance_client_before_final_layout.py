...
class BinanceClient:
    ...
    def place_market_order(self, symbol: str, side: str, quantity: float) -> dict:
        # FINAL GUARD:
        # Binance may BUY only BTC/ETH/SOL.
        # SELL is allowed for legacy cleanup / closing existing holdings.
        allowed_buy = {"BTCUSDT", "ETHUSDT", "SOLUSDT"}
        side_u = str(side).upper()
        if side_u == "BUY" and symbol not in allowed_buy:
            raise RuntimeError(f"BINANCE BUY BLOCKED: {symbol} not allowed. Only BTCUSDT/ETHUSDT/SOLUSDT.")
