...
def _maybe_resume_bot():
    ...
    try:
        ...
        return
    # AUTO-START DISABLED:
    # Bot must start ONLY when operator presses Start.
    return
    # OLD AUTO-START REMOVED. Operator can stop with
  ...
  <div style="font-size:18px;font-weight:700;color:#f0f6fc;font-family:'JetBrains Mono',monospace;">{_proj_str} <span style="font-size:11px;color:#6e7681;">USDT · {r.dynamic_size_pct:.0f}% of ${_live_free:.2f} free</span></div>
    ...
    with st.expander("🔭 Scanner & Exchange (Binance + MEXC)", expanded=False):
        ...
        st.session_state.exchange_mode = st.radio(
            ...
            help="Binance only for BTC/ETH/SOL; MEXC for scanner coins.",
        )
        ...
        st.session_state.use_scanner_symbols = False
        st.caption("🔒 Scanner auto-symbol trading disabled on this droplet.")
...
with st.container():
    ...
    with col_main:
        ...
        st.code("BUY <= -0.05%   |   SELL +0.60%   |   STOP -0.30%   |   COOLDOWN 60s   |   TREND ON   |   VOLUME ON")

        bcol, mcol = st.columns([1,1], gap="large")

        with bcol:
            st.markdown("# 🟡 BINANCE")
            st.caption("DOAR BTCUSDT / ETHUSDT / SOLUSDT — restul LEGACY")
            st.markdown("### 1. Wallet Overview")
            st.metric("Total Balance", f"${float(account_value_usdt or binance_total_usdt or 0):,.2f}")
            st.metric("Cash disponibil", f"${float(binance_free_usdt or 0):,.2f}")
            st.metric("Daily PnL", _fmt_pnl(daily_pnl))
            st.metric("Active Trades", f"{len(open_trades)} / 3")

            st.markdown("### 2. Core Markets")
            _core=["BTCUSDT","ETHUSDT","SOLUSDT"]
            try:
                import bot as _botx
                _state=_botx.get_all_symbol_state()
                st.dataframe([{
                    "Coin":x,
                    "Price":_state.get(x,{}).get("price","—"),
                    "Market-Low":_state.get(x,{}).get("market_low_pct","—"),
                    "Trend":_state.get(x,{}).get("trend","—"),
                    "AI":_state.get(x,{}).get("signal","WAIT"),
                } for x in _core], hide_index=True, width="stretch")
            except Exception as e:
                st.warning(f"AI unavailable: {e}")

            st.markdown("### 3. Binance Chart")
            _bc=st.selectbox("Chart Binance", _core, key="new_binance_chart")
            st.iframe(f"https://s.tradingview.com/widgetembed/?symbol=BINANCE:{_bc}&interval=15&theme=dark&style=1", height=420)

            st.markdown("### 4. Legacy Holdings")
            _legacy=[h for h in account_holdings if (h.get("asset")+"USDT") not in _core and h.get("asset") not in ("USDT","USDC","BUSD","FDUSD","TUSD","DAI")] if account_holdings else []
            st.dataframe(_legacy, hide_index=True, width="stretch")

            st.markdown("### 5. AI Decisions / Reasons")
            for x in _core:
                st.write(f"**{x}** — WAIT / BUY only if Market-Low <= -0.05%, trend ON, volume ON")

        with mcol:
            st.markdown("# 🔵 MEXC")
            st.caption("SCANNER PE MONEDE VOLATILE — MAX 15 TRADE-URI")
            st.markdown("### 1. Wallet Overview")
            try:
                import json, time
                mf=Path("data/mexc_balance.json")
                md=json.loads(mf.read_text()) if mf.exists() else {}
                mu=float(md.get("usdt_available",0) or 0)
            except Exception:
                mu=0
            st.metric("MEXC Balance", f"${mu:,.2f}")
            st.metric("Live Orders", "OFF")
            st.metric("Slots", f"{len(open_trades)} / 15")

            st.markdown("### 2. Live Scanner")
            try:
                import bot as _botx
                _mexc_syms=_botx.resolve_scanner_symbols("mexc", top_n=15)
                _state=_botx.get_all_symbol_state()
                st.dataframe([{
                    "Coin":x,
                    "Price":_state.get(x,{}).get("price","—"),
                    ...
                    "Trend":_state.get(x,{}).get("trend","—"),
                    "AI":_state.get(x,{}).get("signal","WAIT"),
                } for x in _mexc_syms], hide_index=True, width="stretch")
            except Exception as e:
                _mexc_syms=[]
                st.warning(f"Scanner unavailable: {e}")

            st.markdown("### 3. Scanner Coin Charts")
            if _mexc_syms:
                _mc=st.selectbox("Chart MEXC", _mexc_syms, key="new_mexc_chart")
                st.iframe(f"https://s.tradingview.com/widgetembed/?symbol=MEXC:{_mc}&interval=15&theme=dark&style=1", height=420)

            st.markdown("### 4. Active Trades / Rotation / AI")
            st.write("Max 15 simultan · rotație scanner · BUY/SELL/STOP pe universal rules")
...
""", unsafe_allow_html=True)
        ...
        st.markdown("## 🟡 BINANCE — Wallet + BTC/ETH/SOL + Legacy")
        st.info("Binance auto-trade: ONLY BTCUSDT / ETHUSDT / SOLUSDT. Other coins = LEGACY holdings.")
        ...
        try:
            ...
            for _sym in _core:
                _x = _state.get(_sym, {})
                st.write(f"**{_sym}** | price={_x.get('price','—')} | final={_x.get('signal','WAIT')} | reason={_x.get('reason','no reason yet')}")
        except Exception as _e:
            st.caption(f"Binance AI/decision panel unavailable: {_e}")

        if account_holdings:
            _legacy = [h for h in account_holdings if (h.get("asset")+"USDT") not in _core and h.get("asset") not in ("USDT","USDC","BUSD","FDUSD","TUSD","DAI")]
            st.markdown("### Legacy Binance Holdings")
            if _legacy:
                st.dataframe(_legacy, width="stretch")
            else:
                st.caption("No legacy holdings detected.")

        st.divider()

        st.markdown("## 🔵 MEXC — Scanner Market-Low Strategy")
        st.info("MEXC scanner active. Live orders OFF by default. Max 15 trades.")

        st.markdown("### MEXC Wallet Overview")
        try:
            _mexc_file = Path("data/mexc_balance.json")
            if _mexc_file.exists():
                import json, time
                _mexc = json.loads(_mexc_file.read_text())
                _mexc_usdt = float(_mexc.get("usdt_available", 0) or 0)
                _mexc_age = int(time.time() - float(_mexc.get("ts", time.time())))
                st.metric("MEXC USDT", f"${_mexc_usdt:,.2f}", "LIVE ORDERS OFF")
                st.caption(f"MEXC balance updated {_mexc_age}s ago")
            else:
                st.caption("MEXC balance: not loaded yet")
        except Exception as _e:
            st.warning(f"MEXC wallet unavailable: {_e}")

        try:
            import bot as _botx
            _mexc_syms = _botx.resolve_scanner_symbols("mexc", top_n=15)
            st.markdown("### Live Scanner Candidates")
            st.write(", ".join(_mexc_syms) if _mexc_syms else "No candidates yet")
            _state = _botx.get_all_symbol_state()
            for _sym in _mexc_syms:
                _x = _state.get(_sym, {})
                st.write(f"**{_sym}** | price={_x.get('price','—')} | final={_x.get('signal','WAIT')} | reason={_x.get('reason','scanner waiting')}")
        except Exception as _e:
            st.caption(f"MEXC scanner/AI panel unavailable: {_e}")

        st.markdown("### Global Rule")
        st.code("BUY <= -0.05% | SELL +0.60% | STOP -0.30% | Cooldown 60s | Trend ON | Volume ON")
        st.divider()
