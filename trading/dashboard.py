...
st.caption(
    "🧪 **React dashboard (Phase B) is experimental** — production terminal is this "
    "Streamlit UI on port **8501**. Optional React preview: `trading/dashboard-ui` "
...
# Legacy tile builder kept above; strip rendered in Overview tab only.
...
# Verification / bot-status HTML built above — rendered in Overview tab via
# _render_overview_status_strip() (avoids duplicate panels outside tabs).

def _render_overview_status_strip(conn_parts, bot_status_html, reason_html):
    st.markdown(
...
def _render_bot_symbol_overview():
    """Per-symbol bot cards + waiting banner (Overview tab only)."""
    if not bot_running:
        return
    ...
    for _s in _active:
        ...
        _reg_col = {
            "TREND": "#26a69a", "VOLATILE": "#e3b341",
            "RANGE": "#7ce0c2", "DEAD": "#ef5350",
        }.get(_regime, "#6e7681")
        _cards += (
            f'<div style="flex:1 1 0;min-width:220px;background:#0d1117;border:1px solid #1e2736;'
            f'border-radius:8px;padding:10px 12px;">'
            f'  <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:6px;">'
            f'    <span style="font-size:12px;font-weight:800;color:#f0f6fc;">{_s.replace("USDT","")}</span>'
            f'    <span style="font-size:11px;font-weight:800;color:{_sc};">{_sig}</span>'
            f'  </div>'
            f'  <div style="font-size:10px;color:#8b949e;line-height:1.55;">'
            f'    <div><span style="color:#484f58;">SCORE</span> <span style="color:{_score_col};">{_score}</span></div>'
            f'    <div><span style="color:#484f58;">REGIME</span> <span style="color:{_reg_col};">{_regime or "—"}</span></div>'
            f'    <div><span style="color:#484f58;">CHECK</span> {_last_check_s}</div>'
            f'    <div><span style="color:#484f58;">ORDER</span> <span style="color:{_lo_col};">{_last_order_s}</span></div>'
            f'    <div title="{_block_s}"><span style="color:#484f58;">BLOCK</span> <span style="color:{_block_col};">{_block_s}</span></div>'
            f'  </div></div>'
        )
    if _cards:
        st.markdown(
            f'<div style="display:flex;gap:10px;flex-wrap:wrap;padding:10px 0;">{_cards}</div>',
            unsafe_allow_html=True,
        )


def _render_ai_why_panel():
    if not (_ai_on and (_ai_why or _ai_last or _ai_blkr)):
        return
  ...
  <div style="font-size:18px;font-weight:700;color:#f0f6fc;font-family:'JetBrains Mono',monospace;">{_proj_str} <span style="font-size:11px;color:#6e7681;">USDT · {r.dynamic_size_pct:.0f}% of ${_live_free:.2f} free</span></div>
    ...
    with st.expander("💧 Market Low Strategy (LIVE)", expanded=True):
        ...
        _ls.bot_spending_limit_usdt = float(st.number_input(
            ...
            min_value=0.0,
            value=float(getattr(st.session_state.global_risk, "max_total_exposure_usdt", 0.0) or 0.0),
            step=10.0,
            help="Max total USDT the bot may have deployed across all open trades at once.",
            key="dip_spend_limit",
        ))
        st.session_state.global_risk.max_total_exposure_usdt = _ls.bot_spending_limit_usdt
...
def _render_open_trades_summary():
    """Compact all-venue open positions — matches get_open_trades() / API."""
    _sec("💼 Open Trades Summary")
    if not open_trades:
        st.caption("No open positions — same set as /api/trades/open.")
        return
    rows = []
    for t in open_trades:
        ...
        rows.append({
            ...
            "Unrealized": _fmt_pnl(u) if u is not None else "—",
            "Opened": (t.get("open_time") or "")[:16].replace("T", " "),
        })
    st.dataframe(pd.DataFrame(rows), width="stretch", hide_index=True,
                 height=min(40 + 36 * len(rows), 280))
    _bn = sum(1 for t in open_trades if (t.get("exchange") or "binance") == "binance")
    _mx = sum(1 for t in open_trades if t.get("exchange") == "mexc")
    st.caption(f"**{len(open_trades)}** open total · Binance {_bn}/{_binance_cap()} · "
...
def _render_equity_sparkline():
    """Cumulative realized + unrealized P&L curve (real trades only)."""
    _cum = 0.0
    ...
    _sfig.add_trace(go.Scatter(
        x=_eq_times, y=_eq_vals, mode="lines",
    ...
    _sfig.add_trace(go.Scatter(
        x=[_eq_times[-1]], y=[_eq_vals[-1]], mode="markers",
        marker=dict(color=_spark_color, size=7, line=dict(color="#0a0c10", width=2)),
        showlegend=False,
    ...
    _sfig.update_layout(
        ...
        xaxis=dict(visible=True, tickfont=dict(size=9, color="#6e7681")),
        yaxis=dict(visible=True, tickfont=dict(size=9, color="#6e7681"), tickprefix="$"),
        hovermode="x unified",
        uirevision="alphatrade-equity-curve",
    )
    st.markdown("**Cumulative P&L curve (closed + open marks)**")
    st.plotly_chart(_sfig, width="stretch", key="equity_curve_chart",
...
def _render_history_tab(fmt_pnl_fn, fmt_pct_fn):
    ...
    _ex_f = hf1.selectbox("Exchange filter", ["All", "binance", "mexc"], key="hist_ex_f")
    _st_f = hf2.selectbox("Status filter", ["All", "open", "closed"], key="hist_st_f")
    _ty_f = hf3.selectbox("Type filter", ["All", "bot", "manual"], key="hist_ty_f")

    def _filt(trades):
        out = trades
        if _ex_f != "All":
            out = [t for t in out if (t.get("exchange") or "binance") == _ex_f]
        if _st_f != "All":
            out = [t for t in out if t.get("status") == _st_f]
        if _ty_f != "All":
    ...
    with tab_h:
        ...
        if not _shown:
            ...
            st.dataframe(
                pd.DataFrame(dsupport.build_history_rows(_shown, fmt_pnl_fn, fmt_pct_fn)),
...
def _render_performance_tab(fmt_pnl_fn, total_pnl_val, win_rate_val, wins_val):
    """Stats, symbol/hour breakdown, equity curve."""
    _sec("📈 Performance")
    ...
    if closed_trades:
        ...
        _r = _dip_rules()
        st.caption(
            f"Global rules (live_settings): BUY ≤ {_r['buy']:.2f}% · "
            f"TP +{_r['tp']:.2f}% · SL {_r['sl']:.2f}% · "
            f"cooldown {_r['cooldown']}s · trend {'ON' if _r['trend'] else 'OFF'} · "
            f"volume {'ON' if _r['vol'] else 'OFF'}."
        )


def _render_trade_diagnostics_panel():
    """WHY NO TRADE — frequency, blocks, verify/reconcile (observability only)."""
    try:
    ...
    _m4.metric("Min since last trade", f"{_mins:.0f}" if _mins is not None else "—")
    ...
    st.markdown("**Current reason per symbol**")
    if _latest:
        def _fmt_chg(d):
            c = d.get("change_pct")
        ...
        st.info("No decisions recorded yet. Start the bot and give it a few cycles.")

    st.markdown(
        f"**Top block reasons** (cycles: {_cyc['cycles']} · traded: {_cyc['traded']} · "
        f"blocks: {_cyc['total_blocks']})")
    if _topblk:
        st.dataframe(
    ...
    _verify = _bcol1.button("🔍 Verify Binance", width="stretch",
                            disabled=_client_now is None)
    _recon = _bcol2.button("🧹 Reconcile now", width="stretch",
                           disabled=_client_now is None)
    _report = _bcol3.button("📋 Full report", width="stretch")

    if _client_now is None:
        st.caption("Connect a LIVE Binance key to enable verification + reconciliation.")
    ...
    if _verify or _recon or _report:
        ...
        if _recon:
            ...
            if _rc.get("closed"):
                st.warning(f"🧹 Closed {len(_rc['closed'])} ghost trade(s): {_rc['closed']}")
...
def _render_binance_legacy():
    ...
    for _h in (account_holdings or []):
        ...
        _sellable.append(_h)
    if _sellable:
        with st.expander("🧹 Convert a legacy coin to USDT", expanded=True):
...
def _mini_candle_fig(symbol: str, venue: str, interval: str = "5m",
        ...
        try:
            ...
            if _m.buy:
                fig.add_trace(go.Scatter(
                    ...
                    mode="markers+text", text=["BUY"]*len(_m.buy), textposition="top center", showlegend=False,
            ...
            if _sell:
                fig.add_trace(go.Scatter(
                    ...
                    mode="markers+text", text=["SELL"]*len(_sell), textposition="bottom center", showlegend=False,
            ...
            if _stop:
                fig.add_trace(go.Scatter(
                    ...
                    mode="markers+text", text=["STOP"]*len(_stop), textposition="top center", showlegend=False,
    ...
    fig.update_layout(
        ...
        yaxis=dict(visible=True, side='right', tickfont=dict(size=9, color='#8b949e')))
    return fig
