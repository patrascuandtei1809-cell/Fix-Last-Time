...
def create_bot(
    ...
    with _bot_lock:
        ...
        if exchange_mode == "mexc":
            # FINAL ARCHITECTURE:
            # MEXC = scanner symbols, up to MAX_ACTIVE_SYMBOLS.
            # Binance = BTC/ETH/SOL only, same Market-Low rule.
            for sym in sym_list:
                workers[f"{exchange.name}:{sym}"] = _make_worker(exchange, sym)

            if client is not None:
                binance_ex = BinanceExchange(client=client)
                ex_registry.register(binance_ex)
                for sym in ["BTCUSDT", "ETHUSDT", "SOLUSDT"]:
                    workers[f"{binance_ex.name}:{sym}"] = _make_worker(binance_ex, sym)
                print("[BOT] Binance sidecar enabled: BTCUSDT/ETHUSDT/SOLUSDT only", flush=True)
            else:
                print("[BOT] Binance sidecar skipped: no Binance client", flush=True)
        else:
            for sym in sym_list:
                workers[f"{exchange.name}:{sym}"] = _make_worker(exchange, sym)
